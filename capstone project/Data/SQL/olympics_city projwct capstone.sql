...
SELECT e.id, e.event_name, g.season, COUNT(*) AS participation_count
FROM event as e
JOIN Competitor_Event as c ON c.event_id = e.id
join games_competitor as ga on c.competitor_id=ga.id
join games as g on ga.games_id
GROUP BY 1,2,3
...
WITH event_count AS (
	...
	join games_competitor as g on g.id=ce.competitor_id
    JOIN
...
FROM
    event_count ec1
JOIN
    event_count ec2 ON ec1.sport_name = ec2.sport_name AND ec1.gender <> ec2.gender
WHERE
    ec1.num_events > ec2.num_events
ORDER BY
    ec1.sport_name, ec1.gender;
